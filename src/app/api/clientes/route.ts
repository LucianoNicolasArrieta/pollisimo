  ...
  try {
    ...
    const rows = await query<any[]>(
      `SELECT 
         ...
         COALESCE(SUM(CASE WHEN v.estado != 'Cancelado' THEN (CASE WHEN v.peso_kg IS NOT NULL THEN v.peso_kg ELSE COALESCE(v.cantidad_bandejas, 1) * 1.0 END) ELSE 0 END), 0) AS total_kilos,
         COALESCE(SUM(CASE WHEN v.estado != 'Cancelado' THEN (CASE WHEN v.peso_kg IS NOT NULL THEN GREATEST(1, FLOOR(v.peso_kg)) ELSE COALESCE(v.cantidad_bandejas, 1) END) ELSE 0 END), 0) AS total_bandejas,
